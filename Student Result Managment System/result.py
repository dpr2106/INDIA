...
class resultClass:
    def __init__(self,root):
        ...
        lbl_full_marks=Label(self.root,
                            text="Full Marks:",
                            font=("goudy old style",15,"bold"),
        ...
        btn_search=Button(self.root,
                            ...
                            fg="white",
                            cursor="hand2",
                            command=self.search).place(x=500,
                                                  y=100,
                                                  width=120,
                                                  height=28)
        ...
        txt_name=Entry(self.root,
                            ...
                            bg="lightyellow",
                            state="readonly").place(x=280,y=160,width=320)
        txt_course=Entry(self.root,
                            textvariable=self.var_course,
                            font=("goudy old style",20,"bold"),
                            bg="lightyellow",
                            state="readonly").place(x=280,y=220,width=320)
        ...
        self.bg_img=img=self.bg_img.resize((500,300),Image.Resampling.LANCZOS)
        self.bg_img=ImageTk.PhotoImage(self.bg_img)
        self.lbl_bg=Label(self.root,
                          image=self.bg_img).place(x=630,y=100)       
    def fetch_roll(self):
    ...
    def search(self):
        ...
        cur=con.cursor()
        try:
                cur.execute(f"select contact,course from student where roll=?", (self.var_roll.get(),))
                row=cur.fetchone() 
                if row!= None:      
